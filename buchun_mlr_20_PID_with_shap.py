...
- 字体大一号，边框加粗，无网格线
- 所有图表无标题
- 输出路径为 mlr_model_20_result_more
- 所有描述符使用真实名称
- MAE柱状图使用整数刻度
- SHAP图增加至10+张
...
def main():
    ...
    # ========== 输出到 mlr_model_20_result_more ==========
    output_dir = 'mlr_model_20_result_more'
    ...
    print(f"\n{'=' * 80}")
    print("MLR Model - Multiple Linear Regression with 20 descriptors (Enhanced)")
    ...
    try:
        ...
        shap.plots.heatmap(shap.Explanation(values=shap_values,
                                            ...
                                            data=total_X, feature_names=display_names), max_display=20, show=False)
        ax = plt.gca()
    ...
    try:
        ...
        plt.tick_params(labelsize=11)
        plt.tight_layout()
        save_figure_tiff(plt.gcf(), f'{shap_style_dir}/FigS5_shap_importance_bar_20features.tiff')
        plt.close()
        print("    ✓ Saved")
    except Exception as e:
        print(f"    ✗ Failed: {e}")
    ...
    try:
        ...
        report_lines.append(f"  {shap_dir}/plots/FigS1_shap_beeswarm_20features.tiff")
        report_lines.append(f"  {shap_dir}/plots/FigS2_shap_beeswarm_top20.tiff")
        report_lines.append(f"  {shap_dir}/plots/FigS3_shap_heatmap_20features.tiff")
        report_lines.append(f"  {shap_dir}/plots/FigS4_shap_heatmap_top20.tiff")
        report_lines.append(f"  {shap_dir}/plots/FigS5_shap_importance_bar_20features.tiff")
        ...
        report_lines.append("  ✓ Output directory: mlr_model_20_result_more")
