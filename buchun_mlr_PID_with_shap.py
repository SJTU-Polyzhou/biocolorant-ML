...
- 字体大一号，边框加粗，无网格线
- 所有图表无标题
- 输出路径为 mlr_model_70_result_more
- 所有描述符使用真实名称
- MAE柱状图使用整数刻度
- SHAP图增加至10+张
...
def main():
    ...
    # ========== 输出到 mlr_model_70_result_more ==========
    output_dir = 'mlr_model_70_result_more'
    ...
    print(f"\n{'=' * 80}")
    print("MLR Model - Multiple Linear Regression with 70 descriptors (Enhanced)")
    ...
    # ===== SHAP蜂群图 - Top 35特征 (70个太多，显示前35) =====
    print("  Generating FigS1: SHAP beeswarm (Top 35 features)...")
    try:
    ...
    # ===== SHAP热力图 - Top 35特征（高度增加）=====
    print("  Generating FigS3: SHAP heatmap (Top 35 features, height increased)...")
    try:
        setup_cjche_style_heatmap()
        fig_height_cm = 35 * 0.7 + 5
    ...
    # ===== SHAP特征重要性 - Top 35 =====
    print("  Generating FigS5: SHAP importance bar (Top 35 features)...")
    try:
        fig_height_cm = 35 * 0.35 + 3
        fig_height_inch = fig_height_cm / 2.54
        plt.figure(figsize=(16 / 2.54, fig_height_inch))
        top_35_df = importance_df.head(35)
        y_pos = np.arange(len(top_35_df))
        plt.barh(y_pos, top_35_df['mean_abs_shap'].values,
                 color='steelblue', edgecolor='black', alpha=0.8, linewidth=1.2)
        plt.yticks(y_pos, top_35_df['feature'].values, fontsize=6)
    ...
    try:
        ...
        report_lines.append("  ✓ Output directory: mlr_model_70_result_more")
