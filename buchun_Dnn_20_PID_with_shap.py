...
- 热力图高度增加，纵轴标签清晰
- 所有图表无标题
- 输出路径为 dnn_model_20_result_more
- 所有描述符使用真实名称
- 学习曲线为3张独立图
- MAE柱状图使用整数刻度
- SHAP图增加至10+张
...
def main():
    ...
    output_dir = os.path.join('03_modeling', 'DNN_model', 'dnn_model_20_result_more')
    ...
    print(f"\n{'=' * 80}")
    print("DNN Model - Deep Neural Network with 20 descriptors (Enhanced)")
    ...
    # ============================================================
    # 画法1：全部数据阈值 (h* = 3(20+1)/541 ≈ 0.1165)
    # ============================================================
    ...
    print("Generating: 20 features (using real descriptor names)")
    print("=" * 70)
    ...
    try:
        ...
        # 因为总共只有20个特征，所以全部展示，但为了兼容，仍保留top20（即全部）
        ...
        # ===== SHAP蜂群图 - 全部20个特征 =====
        print("  Generating FigS1: SHAP beeswarm (20 features)...")
        try:
            ...
            plt.tight_layout()
            save_figure_tiff(plt.gcf(), f'{shap_style_dir}/FigS1_shap_beeswarm_20features.tiff')
            plt.close()
            print("    ✓ Saved")
        ...
        # ===== SHAP热力图 - 20个特征 =====
        print("  Generating FigS3: SHAP heatmap (20 features, height increased)...")
        try:
            ...
            shap.plots.heatmap(explanation, max_display=20, show=False)
            ...
            plt.tight_layout()
            save_figure_tiff(plt.gcf(), f'{shap_style_dir}/FigS3_shap_heatmap_20features.tiff')
            plt.close()
            setup_cjche_style_larger()
            print("    ✓ Saved")
        except Exception as e:
        ...
        # ===== SHAP特征重要性条形图 - 20个特征 =====
        print("  Generating FigS5: SHAP importance bar (20 features)...")
        try:
            # 自适应高度：每个特征0.4cm + 边距
            ...
            plt.tight_layout()
            save_figure_tiff(fig, f'{shap_style_dir}/FigS5_shap_importance_bar_20features.tiff')
            plt.close()
            print("    ✓ Saved")
        except Exception as e:
            print(f"    ✗ Failed: {e}")

        # ===== SHAP特征重要性条形图 - Top 20（全部） =====
        ...
        report_lines.append(f"  {shap_style_dir}/FigS1_shap_beeswarm_20features.tiff")
        report_lines.append(f"  {shap_style_dir}/FigS2_shap_beeswarm_top20.tiff")
        report_lines.append(f"  {shap_style_dir}/FigS3_shap_heatmap_20features.tiff")
        report_lines.append(f"  {shap_style_dir}/FigS4_shap_heatmap_top20.tiff")
        report_lines.append(f"  {shap_style_dir}/FigS5_shap_importance_bar_20features.tiff")
        ...
        report_lines.append("  ✓ Output directory: dnn_model_20_result_more")
