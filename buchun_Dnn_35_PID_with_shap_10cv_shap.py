# -*- coding: utf-8 -*-
"""
包含SHAP分析的完整DNN模型脚本 - 增强版 + SHAP稳定性分析
...
- 热力图高度增加，纵轴标签清晰
- 所有图表无标题
- 输出路径为 dnn_model_35_result_more_10cv_shap
- 所有描述符使用真实名称
- 学习曲线为3张独立图
- MAE柱状图使用整数刻度
- SHAP图增加至10+张
...
- SHAP特征排序在10折交叉验证上的稳定性分析（统一coolwarm配色）
"""
...
def plot_shap_stability_enhanced(importance_df_stability, consistency_matrix, n_folds, figures_dir):
    """
    生成清晰的SHAP稳定性图（统一coolwarm配色，与SHAP分析一致）
    所有图表无标题，符合CJChE格式要求
    """

    print("\n  Generating SHAP stability plots (coolwarm unified)...")

    # ============================================================
    # 图1：热力图（coolwarm，只标注下三角减少重叠）
    # ============================================================
    fig, ax = plt.subplots(figsize=(11 / 2.54, 9 / 2.54))

    im = ax.imshow(consistency_matrix, cmap='coolwarm', vmin=0.3, vmax=0.85)
    ...
    ax.set_yticklabels(fold_labels, fontsize=11)
    ...
    for i in range(n_folds):
        for j in range(n_folds):
            if i > j and j < i - 1:
                ...
                color = 'white' if val > 0.58 else 'black'
    ...
    # 无标题 - 已移除 ax.set_title()

    for spine in ax.spines.values():
        spine.set_linewidth(1.5)

    plt.tight_layout()
    save_figure_tiff(fig, os.path.join(figures_dir, 'FigS12_shap_stability_heatmap_enhanced.tiff'), dpi=300)
    plt.close()
    print("  ✓ FigS12: Stability heatmap saved (coolwarm)")

    # ============================================================
    # 图2：散点图（coolwarm颜色映射）
    # ============================================================
    fig, ax = plt.subplots(figsize=(13 / 2.54, 10 / 2.54))

    mean_ranks = importance_df_stability['Mean_Rank'].values
    std_ranks = importance_df_stability['Std_Rank'].values
    mean_importance = importance_df_stability['Mean_Importance'].values
    display_names = importance_df_stability['Display_Name'].values

    sizes = mean_importance / mean_importance.max() * 250 + 30

    scatter = ax.scatter(mean_ranks, std_ranks,
                         c=mean_importance, cmap='coolwarm',
                         s=sizes, alpha=0.8,
                         edgecolors='black', linewidth=0.8)

    ax.axhline(y=10, color='gray', linestyle='--', linewidth=1.2, alpha=0.5)
    ax.axvline(x=15, color='gray', linestyle='--', linewidth=1.2, alpha=0.5)

    # 标注关键特征：重要但不稳定 (rank<15, std>12) 和 最重要特征 (rank<8)
    to_annotate = {}
    for i, (rank, std, name) in enumerate(zip(mean_ranks, std_ranks, display_names)):
        if (rank < 15 and std > 12) or (rank < 8):
            to_annotate[i] = (rank, std, name)

    for idx, (rank, std, name) in list(to_annotate.items())[:15]:
        short_name = name[:18] if len(name) > 18 else name
        ax.annotate(short_name, (rank, std),
                    xytext=(5, 5), textcoords='offset points',
                    fontsize=8, alpha=0.9,
                    bbox=dict(boxstyle='round,pad=0.2', facecolor='white',
                              alpha=0.8, edgecolor='gray', linewidth=0.5))

    ax.set_xlabel('Mean Rank (lower = more important)', fontsize=12)
    ax.set_ylabel('Rank Standard Deviation (stability)', fontsize=12)
    ax.tick_params(labelsize=11)
    ...
    plt.tight_layout()
    save_figure_tiff(fig, os.path.join(figures_dir, 'FigS11_shap_stability_scatter_enhanced.tiff'), dpi=300)
    plt.close()
    print("  ✓ FigS11: Stability scatter plot saved (coolwarm)")
    # ============================================================
    # 图2a：散点图（无标签，用于正文展示）
    # ============================================================
    fig, ax = plt.subplots(figsize=(13 / 2.54, 10 / 2.54))
    ...
    plt.tight_layout()
    save_figure_tiff(fig, os.path.join(figures_dir, 'FigS11a_shap_stability_scatter.tiff'), dpi=300)
    plt.close()
    print("  ✓ FigS11a: Stability scatter plot saved (no labels)")

    # ============================================================
    # 图2b：散点图（标注特征名称，用于补充材料）
    # ============================================================
    ...
    for i, (rank, std, name) in enumerate(zip(mean_ranks, std_ranks, display_names)):
        ...
        # 根据重要性调整标注透明度：重要特征更清晰
        alpha_val = 0.9 if rank < 15 else 0.6
        ax.annotate(short_name, (rank, std),
                    xytext=(5, 5), textcoords='offset points',
                    fontsize=7, alpha=alpha_val,
                    bbox=dict(boxstyle='round,pad=0.2', facecolor='white',
                              alpha=0.7, edgecolor='gray', linewidth=0.3))

    # 突出标注top 10特征（红色边框）
    top10_indices = np.argsort(mean_ranks)[:10]
    for idx in top10_indices:
        rank = mean_ranks[idx]
        std = std_ranks[idx]
        name = display_names[idx][:20]
        ax.annotate(name, (rank, std),
                    xytext=(8, 8), textcoords='offset points',
                    fontsize=8, fontweight='bold', color='darkred',
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='white',
                              alpha=0.9, edgecolor='red', linewidth=1.0))
    ...
    plt.tight_layout()
    save_figure_tiff(fig, os.path.join(figures_dir, 'FigS11b_shap_stability_scatter_labeled.tiff'), dpi=300)
    plt.close()
    print("  ✓ FigS11b: Stability scatter plot saved (with labels)")

    # ============================================================
    # 图3：箱线图（coolwarm蓝色渐变）
    # ============================================================
    top_15 = importance_df_stability.head(15)
    ...
    for idx, row in top_15.iterrows():
        ranks = [row[f'Fold_{i + 1}_Rank'] for i in range(n_folds)]
        data_for_box.append(ranks)
        labels_for_box.append(row['Display_Name'][:18])

    bp = ax.boxplot(data_for_box, patch_artist=True,
                    labels=labels_for_box,
                    vert=False, showmeans=True,
                    meanline=True,
                    meanprops=dict(color='red', linewidth=2, alpha=0.8))

    colors = plt.cm.coolwarm(np.linspace(0.6, 0.9, len(data_for_box)))
    ...
    ax.invert_xaxis()
    ax.set_xlabel('Rank (1 = most important)', fontsize=12)
    ...
    plt.tight_layout()
    save_figure_tiff(fig, os.path.join(figures_dir, 'FigS10_shap_stability_boxplot_enhanced.tiff'), dpi=300)
    plt.close()
    print("  ✓ FigS10: Stability boxplot saved (coolwarm gradient)")

    # ============================================================
    # 图4：条形图（coolwarm渐变，最清晰）
    # ============================================================
    top_20 = importance_df_stability.head(20)

    fig, ax = plt.subplots(figsize=(12 / 2.54, 10 / 2.54))

    y_pos = np.arange(len(top_20))
    mean_ranks_top = top_20['Mean_Rank'].values
    std_ranks_top = top_20['Std_Rank'].values
    names_top = top_20['Display_Name'].values

    colors_bar = plt.cm.coolwarm(np.linspace(0.2, 0.9, len(top_20)))[::-1]

    ax.barh(y_pos, mean_ranks_top,
            xerr=std_ranks_top,
            color=colors_bar, alpha=0.8,
            edgecolor='black', linewidth=1.0,
            capsize=3, error_kw=dict(linewidth=1.5, capsize=4, color='black'))

    ax.set_yticks(y_pos)
    ax.set_yticklabels([name[:20] for name in names_top], fontsize=9)
    ax.set_xlabel('Mean Rank (lower = more important)', fontsize=12)
    ax.invert_xaxis()

    for i, (rank, std) in enumerate(zip(mean_ranks_top, std_ranks_top)):
        ax.text(rank + 0.3, i, f'{rank:.1f}±{std:.1f}',
    ...
    plt.tight_layout()
    save_figure_tiff(fig, os.path.join(figures_dir, 'FigS13_shap_stability_barchart.tiff'), dpi=300)
    plt.close()
    print("  ✓ FigS13: Stability bar chart saved (coolwarm gradient)")
    ...
    plt.tight_layout()
    save_figure_tiff(fig, os.path.join(figures_dir, 'FigS12_shap_stability_heatmap_simple.tiff'), dpi=300)
    plt.close()
    print("  ✓ FigS12_simple: Clean heatmap saved (no numeric labels)")


    # ============================================================
    # 统计摘要
    # ============================================================
    print("\n  Stability Statistics Summary:")
    print("  " + "-" * 60)

    upper_tri = []
    for i in range(n_folds):
        for j in range(i + 1, n_folds):
            upper_tri.append(consistency_matrix[i, j])
    upper_tri = np.array(upper_tri)

    print(f"  Pairwise Kendall's τ (mean ± std): {np.mean(upper_tri):.4f} ± {np.std(upper_tri):.4f}")
    print(f"  Range: [{np.min(upper_tri):.4f}, {np.max(upper_tri):.4f}]")

    print(f"\n  Most Stable Features (lowest rank std):")
    for _, row in importance_df_stability.nsmallest(5, 'Std_Rank').iterrows():
        print(f"    {row['Display_Name'][:30]:30s} Rank: {row['Mean_Rank']:.1f} ± {row['Std_Rank']:.1f}")

    print(f"\n  Most Unstable Features (highest rank std):")
    for _, row in importance_df_stability.nlargest(5, 'Std_Rank').iterrows():
        print(f"    {row['Display_Name'][:30]:30s} Rank: {row['Mean_Rank']:.1f} ± {row['Std_Rank']:.1f}")

    return {
        'mean_kendall': np.mean(upper_tri),
        'std_kendall': np.std(upper_tri)
    }


def evaluate_shap_stability(model_params, X_train_raw, y_train, feature_names, display_names,
                            figures_dir, tables_dir, n_folds=10, random_state=42):
    """
    评估SHAP特征排序在10折交叉验证上的稳定性
    """
    print("\n" + "=" * 70)
    print("SHAP STABILITY ANALYSIS ACROSS 10-FOLD CV")
    print("=" * 70)
    ...
    for train_idx, val_idx in kf.split(X_train_raw):
        fold_count += 1
        print(f"\n  Processing fold {fold_count}/{n_folds}...")
        ...
        scaler_X_fold = StandardScaler()
        X_tr_scaled = scaler_X_fold.fit_transform(X_tr)
        X_val_scaled = scaler_X_fold.transform(X_val)

        scaler_y_fold = StandardScaler()
        y_tr_scaled = scaler_y_fold.fit_transform(y_tr.reshape(-1, 1)).ravel()

        model_fold = MLPRegressor(**model_params)
        model_fold.fit(X_tr_scaled, y_tr_scaled)

        def model_predict_fold(x):
            y_pred_scaled = model_fold.predict(x)
            return scaler_y_fold.inverse_transform(y_pred_scaled.reshape(-1, 1)).ravel()

        background_size = min(50, len(X_tr_scaled))
        background_indices = np.random.choice(len(X_tr_scaled), background_size, replace=False)
        background = X_tr_scaled[background_indices]

        explainer = shap.KernelExplainer(model_predict_fold, background)

        n_samples = min(100, len(X_val_scaled))
        sample_indices = np.random.choice(len(X_val_scaled), n_samples, replace=False)
        X_sample = X_val_scaled[sample_indices]

        shap_values_fold = explainer.shap_values(X_sample, nsamples=100)

        importance_fold = np.abs(shap_values_fold).mean(axis=0)
        fold_importances.append(importance_fold)
        fold_shap_values.append(shap_values_fold)
    ...
    importance_df_stability = pd.DataFrame({
        'Feature_Code': feature_names,
        'Display_Name': display_names,
    })

    for i, imp in enumerate(fold_importances):
        importance_df_stability[f'Fold_{i + 1}_Importance'] = imp

    rank_cols = []
    for i in range(n_folds):
        rank_col = f'Fold_{i + 1}_Rank'
        importance_df_stability[rank_col] = importance_df_stability[f'Fold_{i + 1}_Importance'].rank(
            method='dense', ascending=False
        ).astype(int)
        rank_cols.append(rank_col)

    rank_array = importance_df_stability[rank_cols].values
    importance_df_stability['Mean_Rank'] = np.mean(rank_array, axis=1)
    importance_df_stability['Std_Rank'] = np.std(rank_array, axis=1)
    importance_df_stability['Mean_Importance'] = np.mean(
        importance_df_stability[[f'Fold_{i + 1}_Importance' for i in range(n_folds)]].values, axis=1
    )
    importance_df_stability['Std_Importance'] = np.std(
        importance_df_stability[[f'Fold_{i + 1}_Importance' for i in range(n_folds)]].values, axis=1
    )

    importance_df_stability = importance_df_stability.sort_values('Mean_Importance', ascending=False)
    importance_df_stability['Overall_Rank'] = range(1, len(importance_df_stability) + 1)

    # 计算一致性
    consistency_metrics = []
    for i in range(n_folds):
        for j in range(i + 1, n_folds):
            rank_i = importance_df_stability[f'Fold_{i + 1}_Rank'].values
            rank_j = importance_df_stability[f'Fold_{j + 1}_Rank'].values
            tau, _ = kendalltau(rank_i, rank_j)
            rho, _ = spearmanr(rank_i, rank_j)
    ...
    # 构建一致性矩阵
    consistency_matrix = np.ones((n_folds, n_folds))
    for idx, row in consistency_df.iterrows():
        i, j = row['Fold_Pair'].split('-')
        i, j = int(i) - 1, int(j) - 1
        consistency_matrix[i, j] = row['Kendall_Tau']
        consistency_matrix[j, i] = row['Kendall_Tau']

    print(f"\n  Number of folds: {n_folds}")
    print(f"  Number of features: {len(feature_names)}")
    print(f"\n  Consistency metrics (pairwise):")
    print(
        f"    Mean Kendall's τ: {consistency_df['Kendall_Tau'].mean():.4f} ± {consistency_df['Kendall_Tau'].std():.4f}")
    print(
        f"    Mean Spearman's ρ: {consistency_df['Spearman_Rho'].mean():.4f} ± {consistency_df['Spearman_Rho'].std():.4f}")

    # 生成稳定性图（统一coolwarm配色）
    plot_shap_stability_enhanced(
        importance_df_stability,
        consistency_matrix,
        n_folds,
        figures_dir
    )
    ...
    # 保存Excel
    stability_excel_path = os.path.join(tables_dir, 'TableS7_shap_stability_analysis.xlsx')
    with pd.ExcelWriter(stability_excel_path, engine='openpyxl') as writer:
        importance_df_stability.to_excel(writer, sheet_name='Feature_Stability', index=False)
        consistency_df.to_excel(writer, sheet_name='Pairwise_Consistency', index=False)
        ...
        summary_data = {
            ...
            'Value': [
                n_folds,
                len(feature_names),
                consistency_df['Kendall_Tau'].mean(),
                consistency_df['Kendall_Tau'].std(),
                consistency_df['Spearman_Rho'].mean(),
                consistency_df['Spearman_Rho'].std(),
                consistency_df['Kendall_Tau'].min(),
                consistency_df['Kendall_Tau'].max()
            ]
        }
        summary_df = pd.DataFrame(summary_data)
        summary_df.to_excel(writer, sheet_name='Summary', index=False)

    print(f"\n  ✓ SHAP stability results saved to: {stability_excel_path}")

    return {
        'importance_df': importance_df_stability,
        'consistency_df': consistency_df,
        'fold_importances': fold_importances,
        'fold_shap_values': fold_shap_values,
        'mean_kendall_tau': consistency_df['Kendall_Tau'].mean(),
        'std_kendall_tau': consistency_df['Kendall_Tau'].std()
    }
...
def main():
    ...
    # 输出目录 - 包含10cv_shap标识
    output_dir = os.path.join('03_modeling', 'DNN_model', 'dnn_model_35_result_more_10cv_shap')
    ...
    print(f"\n{'=' * 80}")
    print("DNN Model - Deep Neural Network with 35 descriptors (Enhanced + SHAP Stability)")
    ...
    ax2.barh(range(len(vif_sorted)), vif_sorted['VIF'].values,
    ...
    try:
        ...
        # ================= 19. SHAP稳定性分析 =================
        print(f"\nStep 18: SHAP Stability Analysis (10-fold CV)")
        print("-" * 60)

        shap_stability_results = evaluate_shap_stability(
            final_params,
            X_train_raw,
            y_train.ravel(),
            feature_names,
        ...
        if np.sum(outliers_full) > 0:
            ...
            report_lines.append("")
        report_lines.append("SHAP STABILITY ANALYSIS (10-fold CV):")
        report_lines.append("-" * 50)
        report_lines.append(f"Mean Kendall's τ: {shap_stability_results['mean_kendall_tau']:.4f} "
                            f"± {shap_stability_results['std_kendall_tau']:.4f}")
        report_lines.append("")
        ...
        report_lines.append(f"  {tables_dir}/TableS7_shap_stability_analysis.xlsx")
        report_lines.append("")
        ...
        report_lines.append(f"  {figures_dir}/FigS10_shap_stability_boxplot_enhanced.tiff")
        report_lines.append(f"  {figures_dir}/FigS11_shap_stability_scatter_enhanced.tiff")
        report_lines.append(f"  {figures_dir}/FigS12_shap_stability_heatmap_enhanced.tiff")
        report_lines.append(f"  {figures_dir}/FigS12_shap_stability_heatmap_simple.tiff")
        report_lines.append(f"  {figures_dir}/FigS13_shap_stability_barchart.tiff")
        report_lines.append("")
        ...
        report_lines.append("  ✓ Output directory: dnn_model_35_result_more_10cv_shap")
        ...
        report_lines.append("  ✓ NEW: SHAP stability analysis across 10-fold CV")
        report_lines.append("  ✓ NEW: Stability boxplot, scatter plot, and heatmap (coolwarm)")
        report_lines.append("  ✓ NEW: Kendall's τ and Spearman's ρ consistency metrics")
        report_lines.append("=" * 80)
